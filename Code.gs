...
function runIRV() {
  ...
  var voteSheet = ss.getSheets()[0];
  var resultSheet = ss.getSheetByName('Resultat');
  ...
  function eliminateBottom(candidates, level, round) {
    ...
    if (bottom.length == 1) {
      ...
      logToSheet([bottom[0] + " was eliminated in round " + round.toString() + ", on level " + level.toString()]);
    }
    else {
  ...
  while (!done) { 
    ...
    if (tally[voteSorted[0]] > totalTally/2) {
      ...
      logToSheet(["No winner yet."]);
    }  
    
    // Log the current tally
    for (var currVote in tally) {
      logToSheet(["", "Votes for "+ currVote + ": " + tally[currVote]]);
    };
